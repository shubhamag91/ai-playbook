(function() {
  function init() {
    ...
    // Persisted UI transcript (survives page navigation within the tab session)
    var STORE_KEY = 'cw-transcript';
    var transcript = [];

    // Gather current-page context so the bot can act as a page-aware copilot
    function getPageContext() {
      var headings = [];
      document.querySelectorAll('main h2, main h3').forEach(function (h) {
        var t = (h.textContent || '').trim();
        if (t && headings.length < 8) headings.push(t);
      });
      return { title: document.title, url: location.href, headings: headings.join('; ') };
    }

    function persist() {
      try { sessionStorage.setItem(STORE_KEY, JSON.stringify(transcript)); } catch (e) { /* ignore */ }
    }

    // Replay a persisted transcript after a page navigation within the tab session
    function restore() {
      var saved;
      try { saved = JSON.parse(sessionStorage.getItem(STORE_KEY) || '[]'); } catch (e) { saved = []; }
      if (!saved || !saved.length) return;
      transcript = saved.slice();
      if (welcomeMsg && welcomeMsg.parentNode) welcomeMsg.parentNode.removeChild(welcomeMsg);
      started = true;
      hideSuggestions();
      saved.forEach(function (m) {
        addMsg(m.text, m.isUser, m.source, m.sources, true);
      });
    ...
    // Page-aware quick actions, prepended to the suggestion list
    if (suggestions) {
      var ctxActions = [
        { label: '✨ Explain this page', q: 'Explain what this page covers in simple terms.' },
        { label: '📝 Quiz me on this page', q: 'Ask me 3 quick questions to test my understanding of this page, one at a time.' }
      ];
      ctxActions.reverse().forEach(function (a) {
        var b = document.createElement('button');
        b.className = 'cw-suggestion cw-ctx';
        b.setAttribute('data-q', a.q);
        b.textContent = a.label;
        suggestions.insertBefore(b, suggestions.firstChild);
      });
    }
    ...
    function addMsg(text, isUser, source, sources, isRestore) {
      ...
      // Persist for restore-after-navigation (skip while replaying a restore)
      if (!isRestore) {
        transcript.push({ text: text, isUser: isUser, source: source, sources: sources });
        if (transcript.length > 20) transcript.splice(0, transcript.length - 20);
        persist();
      }
    }
    ...
    input.addEventListener('keydown', function(e) {
      // Enter sends; Shift+Enter inserts a newline in the multi-line input
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); ask(); }
    });
    send.addEventListener('click', ask);

    // Restore any in-progress conversation from this tab session
    restore();
  }
